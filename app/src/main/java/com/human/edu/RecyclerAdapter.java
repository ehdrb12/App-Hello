...
/**
 * RecyclerAdapter클래스는 recyclerView콤포넌트와 스프링RestAPI를 바인딩 시키는 기능.
...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapter.ItemViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ItemViewHolder holder, int position) {
        //구현은 개발자가 코딩함(아래)
        MemberVO studentVO = (MemberVO) mList.get(position);//1개레코드저장
        //리사이클러뷰의 recyclerview_item.xml디자인에 데이터를 출력합니다.
        holder.itemId.setText(studentVO.getUser_id());
        holder.itemName.setText(studentVO.getUser_name());
        holder.itemEmail.setText(studentVO.getEmail());
    }
    ...
    public class ItemViewHolder extends RecyclerView.ViewHolder {
        //멤버변수선언:클래스전역변수
        private TextView itemId;//홀더내에 객체중 아이디 아이템
        private TextView itemName;//이름 아이템
        private TextView itemEmail;//이메일 아이템
        public ItemViewHolder(@NonNull View itemView) {//new 생성자 메서드
            ...
            itemId = itemView.findViewById(R.id.item_id);//xml의 콤포넌트를 객체화 시킴
